...
def get_data(path, mode='json'):
    """
    Lê um arquivo de dados linha por linha.
    
    Args:
        path (str): O caminho para o arquivo a ser lido.
        mode (str): O modo de leitura. 'json' para arquivos com um JSON por linha, 
                    outro valor para texto simples.

    Returns:
        list: Uma lista contendo os dados do arquivo.
    """
    result = []
    with open(path, 'r', encoding='utf-8') as src:
        if mode == 'json': # trata cada linha como um objeto json
            for line in tqdm(src):
                line = json.loads(line) # converte a string da linha pra um dicionário
                result.append(line)
        else:
            for line in tqdm(src):
...
class KShotTaskSampler(Sampler):
    """
    Um Sampler do PyTorch que gera lotes de tarefas n-shot, k-way.
    Cada tarefa contém um conjunto de suporte e um conjunto de consulta.
    """
    def __init__(self,
                 dataset: torch.utils.data.Dataset,
                 episodes_per_epoch: int = None,
                 k: int = None, # k-way: número de classes por episódio
                 n: int = None, # n-shot: número de amostras de suporte por classe
                 q: int = None, # q-query: número de amostras de consulta por classe
                 num_tasks: int = 1,
        ...
        if num_tasks < 1:
            raise ValueError('num_tasks deve ser > 0.')
    ...
    def __len__(self):
        """ Retorna o número de episódios em uma época. """
...
class MyDataset(Dataset):
    ...
    def index_subset(path):
        """
        Método estático para ler o arquivo e estruturar os dados.
        
        Args:
            path (str): Caminho para o arquivo de dados.

        Returns:
            list: Uma lista de dicionários, cada um representando uma amostra.
        """
        texts = []
...
def write_data(datas, path):
    """
    Escreve uma lista de dados em um arquivo, onde cada item da lista
    se torna uma linha formatada como JSON.
    
    Args:
        datas (list): A lista de dados (geralmente dicionários) a ser escrita.
        path (str): O caminho do arquivo de saída.
    """
...
"""def get_label_dict(args):
    ...
    Args:
        args: Um objeto (geralmente argparse.Namespace) que contém o nome do dataset
              no atributo args.dataset.
